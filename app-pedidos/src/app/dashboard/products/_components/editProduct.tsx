...
interface EditProductFormProps {
  closeModal: () => void;
  handleUpdateProduct: (id: string, product: IProduct) => void;
...
const EditProductForm: React.FC<EditProductFormProps> = ({ closeModal, handleUpdateProduct, selectedProduct }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  useEffect(() => {
    ...
    setValue("inventario", selectedProduct.inventario);
  }, [selectedProduct, setValue]);

  const onSubmit: SubmitHandler<FormData> = async (data) => {
    await handleUpdateProduct(selectedProduct._id, data);
    reset();
    closeModal();
  };
